...
from ymbot_async.api.schemas import Chat, GetUpdatesResponse, Sender, Update
...
class TestPollerFetchUpdates:
    ...
    async def test_fetch_updates_propagates_error(self, bot_config, mock_api_client):
        ...
        mock_api_client.get_updates = AsyncMock(side_effect=Exception("API error"))
        poller = Poller(mock_api_client, bot_config)
...
class TestPollerEmptyResponses:
    ...
    async def test_response_resets_counter(self, mock_api_client):
        ...
        mock_api_client.get_updates = AsyncMock(side_effect=get_updates_with_empty_then_updates)
...
class TestPollerErrorHandling:
    ...
    async def test_continues_after_error(self, mock_api_client):
        ...
        mock_api_client.get_updates = AsyncMock(side_effect=get_updates_with_error)
