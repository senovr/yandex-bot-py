...
class TestHttpxTransport:
    ...
    @pytest.mark.asyncio
    async def test_request_success(self, transport):
        """Test successful JSON request"""
        async with transport:
            mock_response = MagicMock()
            mock_response.status_code = 200
            mock_response.json = MagicMock(return_value={"ok": True, "result": "data"})
    ...
    @pytest.mark.asyncio
    async def test_request_with_params(self, transport):
    ...
    @pytest.mark.asyncio
    async def test_request_with_body(self, transport):
